    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        for (Element element : roundEnv.getElementsAnnotatedWith(Function.class)) {
            if (element.getKind() == ElementKind.METHOD) {
                ...
                Function operation = element.getAnnotation(Function.class);
                try {
                    deploymentGenerator.generateConfig(providerName, configOutputDir,   
                            new HandlerInfo( operation.name(), handlerSimpleName, handlerFQN, packageName, methodName, inputType.toString(), outputType.toString()));
